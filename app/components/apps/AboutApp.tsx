...
export default function AboutApp({ isDark }: { isDark: boolean }) {
  ...
  const cardBg = isDark ? "bg-white/5 border-white/5" : "bg-cream-dark/50 border-white/60";

  return (
    <div className="space-y-6">
      <div className="flex items-start gap-6">
        <div className={`w-24 h-24 rounded-2xl flex items-center justify-center text-3xl font-bold shadow-lg shrink-0 text-white ${isDark ? "bg-gradient-to-br from-cyan-400 to-blue-600" : "bg-gradient-to-br from-folder-blue to-blue-600"}`}>
          IO
        </div>
        <div>
          <h2 className={`text-2xl font-bold mb-1 ${text}`}>Izi Oussama</h2>
          <p className={`font-mono text-sm mb-3 ${accent}`}>Full Stack Developer & Designer</p>
          <p className={`text-sm leading-relaxed ${textLight}`}>
            I craft digital experiences where technical precision meets visual artistry. 
            Building immersive web applications with modern technologies.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <h3 className={`text-sm font-semibold uppercase tracking-wider ${textLight}`}>What I Do</h3>
        {[
          ...
          <motion.div
            ...
            className={`flex items-center gap-4 p-3 rounded-xl transition-colors ${isDark ? "bg-white/5 hover:bg-white/10" : "bg-cream-dark/30 hover:bg-cream-dark/50"}`}
          >
